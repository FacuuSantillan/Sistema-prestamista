...
import { X, Search, User, Briefcase, Receipt, CreditCard, ChevronRight } from 'lucide-react'
import { supabase } from '../../lib/supabaseClient'

export default function ModalDirectorio({ isOpen, onClose, tipoInicial = 'clientes' }) {
  ...
  // 1. Resetear el estado cuando se abre el modal
  useEffect(() => {
    if (isOpen) {
      setTipo(tipoInicial)
  ...
  // 2. Cargar lista principal según la pestaña activa
  useEffect(() => {
    if (!isOpen) return
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (tipo === 'inversionistas') {
          query = query.eq('es_inversionista', true)
        } else {
          // Para que en Clientes no aparezcan los inversionistas puros:
          query = query.or('es_inversionista.is.null,es_inversionista.eq.false')
        }
        ...
        setLista([]) // 👈 Limpia la lista si falla la consulta
      } finally {
        setLoading(false)
      }
    }
  ...
  // 3. Función independiente para cargar el detalle del perfil
  const cargarDetallePerfil = async (item, tipoActual) => {
    ...
    try {
      if (tipoActual === 'clientes') {
        ...
        // 2. Cargar Historial de Cobros del Cliente
        ...
        // --- VISTA INVERSIONISTA ---
        // 1. Cargar Préstamos asociados a este Inversionista
        const { data: prestamosData, error: errP } = await supabase
          .from('prestamos')
          .select('*')
        ...
        // 2. Cargar Clientes únicos financiados por este Inversionista
        if (prestamosData && prestamosData.length > 0) {
          const clienteIds = [...new Set(prestamosData.map((p) => p.cliente_id))].filter(Boolean)
  ...
  // Formateador sencillo de fecha ISO a DD/MM/YYYY
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1">
              {loading ? (
                ...
                lista.map((item) => (
                  ...
                  >
                    <div>
                      <h4 className="font-bold text-sm leading-tight">{item.nombre_completo}</h4>
          ...
          <div className="md:col-span-8 flex flex-col h-full overflow-y-auto pl-0 md:pl-2 pr-1">
            {!itemSeleccionado ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400">
                ...
                <p className="text-sm font-medium">Seleccioná un perfil de la izquierda para ver su información</p>
              </div>
            ) : fetchingDetalle ? (
              ...
              <div className="space-y-5">
                ...
                {/* Datos Personales */}
                <div className="p-5 rounded-2xl bg-white border border-line shadow-xs">
                  ...
                  <h3 className="text-2xl font-serif font-bold text-slate-900 mt-0.5">
                    {itemSeleccionado.nombre_completo}
                  </h3>
                  ...
                  <>
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 mb-2.5 flex items-center gap-2">
                        ...
                        Clientes Financiados ({clientesRelacionados.length})
                      </h4>
